...
By default, uses rules from the https://github.com/daymor-dev/ultima-nexus-jvm-check[ultima-nexus-jvm-check] artifact:

* **Main sources** (`pmdMain`): Uses `pmdRuleset.xml` — strict rules for production code
* **Test sources** (`pmdTest`, `pmdIntegrationTest`, `pmdFunctionalTest`, `pmdPerformanceTest`): Uses `pmdRuleset-test.xml` — relaxed rules that exclude false positives common in test code

=== Test Ruleset for Main Sources

Modules whose `src/main/java` contains test infrastructure code (e.g., test support modules) can opt in to the relaxed test ruleset for `pmdMain` by setting `pmd.useTestRulesetForMain=true` in `gradle.properties` or via the extension.
This avoids false positives from strict production rules on code that is inherently test-oriented.

Custom configuration is only needed to override defaults.
...
spotbugsConfig {
    ...
    useAuxClasspath = true    // Include full classpath (runtime and compileOnly) for deeper analysis
}
----
...
* Optional auxiliary classpath (runtime and compileOnly) for resolving transitive classes during analysis
...
|`pmd.ruleSetFile` |_(auto)_ |`pmd` |Path to custom PMD ruleset XML (main sources)
|`pmd.testRuleSetFile` |_(auto)_ |`pmd` |Path to custom PMD ruleset XML (test sources)
|`pmd.useTestRulesetForMain` |`false` |`pmd` |Use the test ruleset for `pmdMain`
...
|`spotbugs.useAuxClasspath` |`false` |`spotbugs` |Include full classpath (runtime and compileOnly) as auxiliary classpath for deeper analysis
...
2. Checkstyle
3. PMD (preceded by `mergePmdFilter`)
4. SpotBugs (preceded by `mergeSpotBugsFilter`)
