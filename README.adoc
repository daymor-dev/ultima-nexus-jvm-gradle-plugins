...
image::ultima-nexus-logo.png[Ultima Nexus JVM Gradle Plugins,200]
...
* *Bean Mapping*: MapStruct with optional Spring component-model overlay
...
├── starter                (java-library + lifecycle + identity + dependency-rules + publish) [dependency aggregation]
├── jvm-complete           (gradle-project + check + test + report) [language-agnostic]
...
├── java-complete-application  (check + test + report + compile-java + application + jspecify bundle)
└── java-complete-library      (check + test + report + compile-java + java-library + publish + jspecify bundle)
...
├── spring-boot-complete-application   (check + test + report + compile-java + spring-boot-application + jspecify bundle)
└── spring-boot-complete-library       (check + test + report + compile-java + spring-boot-library + publish + jspecify bundle)
...
|Spring Boot: Application/library mode, test dependencies, DevTools, Docker Compose

|link:plugins-frontend/README.adoc[plugins-frontend]
|Frontend: Vaadin and htmx rendering backends, each with a framework-neutral feature, a Spring layer, and a framework bundle

|link:plugins-persistence/README.adoc[plugins-persistence]
|Persistence: Offline jOOQ codegen, build-time Flyway and Liquibase migration wiring

|link:plugins-integration/README.adoc[plugins-integration]
...
|link:plugins-mapper/README.adoc[plugins-mapper]
|Object mapping: MapStruct annotation processor wiring with optional Spring component-model overlay

|link:plugins-serialization/README.adoc[plugins-serialization]
...
|Miscellaneous: Aggregation, version platform, schema docs, `jvm-complete` bundle, full `ultima-nexus-jvm` bundle
|===
