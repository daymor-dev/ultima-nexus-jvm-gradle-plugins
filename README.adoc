...
* *Spring Boot* - Preconfigured dependencies and application setup (auto-detected)
...
[plugins]
ultimanexus-bundle-complete = { id = "dev.daymor.ultimanexus.jvm.gradle.bundle.ultima-nexus-complete", version.ref = "ultima-nexus-plugins" }
----
...
==== Step 3: Project Properties (`gradle.properties`)
...
==== Spring Boot Projects (Java)

The quickest way - includes everything:
...
For Java projects without Spring Boot:

[source,kotlin]
----
plugins {
    alias(libs.plugins.ultimanexus.jvm.bundle.java.complete)
    alias(libs.plugins.ultimanexus.jvm.bundle.lombok)  // optional
}
----
...
For version management platforms. Spring Boot BOM is auto-included when `org.springframework.boot` plugin is applied:

[source,kotlin]
----
plugins {
    alias(libs.plugins.ultimanexus.jvm.bundle.platform)
    // Spring Boot BOM automatically included if this is present:
...
├── ultima-nexus-jvm       (project-structure + git-hooks + build-cache + develocity)
└── ultima-nexus-complete  (ALL-IN-ONE: settings + root + all subprojects)
...
├── platform             (version-platform + identity) [BOM projects only, Spring Boot BOM auto-detected]
...
├── spring-boot-simple     (java-simple + spring-boot-unified + spring-boot-test)
└── spring-boot-complete   (check + test + report + compile-java + publish + spring-boot-unified) [app/lib mode]

Level 4 - Full Bundle:
└── ultima-nexus       (spring-boot-complete + lombok + documentation [auto-detected])
----
...
|Settings: Project structure, git hooks, build cache, Develocity, Maven Central aggregation

|link:plugins-misc/README.adoc[plugins-misc]
|Miscellaneous: Aggregation, version platform, `jvm-complete` bundle, full `ultima-nexus` bundle
|===
