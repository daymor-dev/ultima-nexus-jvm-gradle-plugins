...
|JMH baseline-regression tracking (`benchmarkRegression` / `benchmarkBaselineUpdate`)
...
[plugins]
ultimanexus-jvm-bundle-test = { id = "dev.daymor.ultimanexus.jvm.gradle.bundle.test", version.ref = "ultima-nexus-plugins" }
ultimanexus-jvm-test-test = { id = "dev.daymor.ultimanexus.jvm.gradle.test.test", version.ref = "ultima-nexus-plugins" }
ultimanexus-jvm-test-test-suites = { id = "dev.daymor.ultimanexus.jvm.gradle.test.test-suites", version.ref = "ultima-nexus-plugins" }
ultimanexus-jvm-test-test-fixtures = { id = "dev.daymor.ultimanexus.jvm.gradle.test.test-fixtures", version.ref = "ultima-nexus-plugins" }
----
...
* JMH benchmark regression tracking (`benchmarkRegression` / `benchmarkBaselineUpdate`)
...
Turns the JMH JSON results written by the `performanceTest` suite
(`build/reports/jmh/*.json`) into baseline-regression tracking.

|===
|Task |Behaviour

|`benchmarkBaselineUpdate`
|Promotes the latest results into the committed `perf-baselines/` directory.

|`benchmarkRegression`
|Compares the latest results against `perf-baselines/` (by benchmark name, on
 `primaryMetric.score`) and reports improvements / regressions. Wired as a
 finalizer of `performanceTest`.
|===

A benchmark regresses when its score moves in the worse direction (lower for
throughput modes, higher for time-based modes) by more than the threshold
(default 20%). Because absolute JMH scores are machine-dependent, the check is
*advisory by default* — it logs warnings and keeps the build green. On a stable
benchmark host, opt into hard failure:

[source,kotlin]
----
benchmark {
    thresholdPercent = 20.0   // relative tolerance
    failOnRegression = true   // fail the build on a regression
}
----
...
----
./gradlew performanceTest
./gradlew benchmarkBaselineUpdate   # commit perf-baselines/*.json
----
...
testSuites {
    ...
    suiteConfig("performanceTest") {
        ...
        useJacoco.set(false)           // Disable coverage for benchmarks
    }
...
JaCoCo code coverage is enabled by default for all suites EXCEPT performance tests. This tracks test coverage for reporting and verification.

Performance tests disable JaCoCo by default because JMH forks its own JVM processes, making JaCoCo instrumentation ineffective for benchmark code.
...
|`test.useJacoco` |`true` |`test-suites` |Enable JaCoCo code coverage
...
|`test.suite.<name>.useJacoco` |`true` (except performanceTest: `false`) |`test-suites` |Per-suite JaCoCo coverage setting
|===
