...
* **SBOM Support**: Runtime dependencies for CycloneDX SBOM generation (from plugins-report)
...
* **Automatic Plugin Version Resolution**: No `resolutionStrategy` configuration needed
...
NOTE: Settings plugins cannot use version catalogs because the catalog is not available during settings evaluation. You must specify the version directly in the `plugins {}` block.
...
* SBOM runtime dependencies (for report.sbom plugin)
...
* SBOM runtime dependencies (required for SBOM generation)
* Automatic plugin version resolution
...
The settings bundle plugins (`ultima-nexus-jvm-settings` and `ultima-nexus-jvm-complete`) automatically configure plugin version resolution. This means:

* **No `pluginManagement` block required** in your `settings.gradle.kts`
* **No `resolutionStrategy` configuration needed**
* All Ultima Nexus JVM plugins automatically use the same version as the settings plugin

=== How It Works

When you apply the settings plugin with a version:
...
The plugin automatically configures `pluginManagement.resolutionStrategy` to resolve all `dev.daymor.ultimanexus.jvm.gradle.*` plugins to version `1.0.0`.
