...
|`dev.daymor.ultimanexus.jvm.gradle.bundle.ultima-nexus-complete`
...
== Settings Bundle (ultima-nexus-jvm)
...
== Complete Bundle (ultima-nexus-complete)
...
* Gets `bundle.platform` instead of `ultima-nexus`

*All other subprojects (automatically applied):*

* Full `ultima-nexus` bundle:
...
----
my-project/
├── settings.gradle.kts    # Just apply ultima-nexus-complete here
...
* You have a single-module project (use `ultima-nexus` bundle instead)
...
----
plugins {
    alias(libs.plugins.ultimanexus.jvm.feature.build.cache)
}
----
...
----
plugins {
    alias(libs.plugins.ultimanexus.jvm.report.develocity)
}
----
