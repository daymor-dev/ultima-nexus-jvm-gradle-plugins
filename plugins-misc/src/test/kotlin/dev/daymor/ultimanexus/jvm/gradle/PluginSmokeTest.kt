...
class PluginSmokeTest {
    ...
    @Nested
    inner class AggregationPlugin {

        @Test
        fun `aggregation plugin applies successfully`() {
            fixture()
                .withSettings("rootProject.name = \"test-project\"")
                .withBuildScript(
                    """
                    plugins {
                        id("${PluginIds.Feature.AGGREGATION}")
                    }
                    """.trimIndent()
                )
    ...
    @Nested
    inner class VersionPlatformPlugin {

        @Test
        fun `version-platform plugin applies successfully`() {
            fixture()
                .withSettings("rootProject.name = \"test-project\"")
                .withBuildScript(
                    """
                    plugins {
                        id("${PluginIds.Feature.VERSION_PLATFORM}")
                    }
                    """.trimIndent()
                )
    ...
    @Nested
    inner class UseAllCatalogVersionsPlugin {

        @Test
        fun `use-all-catalog-versions plugin applies successfully`() {
            fixture()
                .withSettings("rootProject.name = \"test-project\"")
                .withBuildScript(
                    """
                    plugins {
                        `java-platform`
                        id("${PluginIds.Feature.USE_ALL_CATALOG_VERSIONS}")
                    }
                    """.trimIndent()
                )
    ...
    @Nested
    inner class GradleProjectRootBundle {

        @Test
        fun `gradle-project-root bundle applies successfully`() {
            fixture()
                .withSettings("rootProject.name = \"test-project\"")
                .withBuildScript(
                    """
                    plugins {
                        id("${PluginIds.Bundle.GRADLE_PROJECT_ROOT}")
                    }
                    """.trimIndent()
                )
