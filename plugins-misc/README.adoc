...
* **Schema Docs**: Collect generated Mermaid ER (`.mmd`) and DBML (`.dbml`) schema documentation
...
|`dev.daymor.ultimanexus.jvm.gradle.feature.schema-docs`
|Opt-in `generateSchemaDocs` task collecting generated Mermaid ER and DBML schema documentation
...
|All-in-one Ultima Nexus JVM application (Spring Boot + GraalVM + Lombok + auto-included starter, annotations, and processor)

|`dev.daymor.ultimanexus.jvm.gradle.bundle.ultima-nexus-jvm-library`
|Complete Ultima Nexus JVM Framework for libraries (Spring Boot + publishing)
...
All-in-one bundle — no dependencies block needed:
...
// Zero dependencies — starter, annotations, and processor are auto-included
----

The plugin automatically adds:

* **Starter** (`implementation`) — the archetype's starter, selected by `ultimaNexusJvm.archetype`
* **Annotations** (`compileOnly`) — `@GenerateStack`, `@GenerateApplication`, etc.
* **Processor** (`annotationProcessor`) — selected from `ultimaNexusJvm.archetype` and `ultimaNexusJvm.useHibernate`
* **Schema docs** (`generateSchemaDocs` task) — opt-in collection of the processor's generated Mermaid ER and DBML files into `build/schema-docs/`
...
|`true`
|Use Hibernate generation. Set `false` for plain JPA identity methods. Ignored for the reactive archetypes (R2DBC persistence).
|===
...
|`starter-spring-classic-backend` / `-fullstack`

|`performance-backend` / `performance-fullstack`
|`starter-spring-performance-backend` / `-fullstack`

|`reactive-backend` / `reactive-fullstack`
|`starter-spring-reactive-backend` / `-fullstack`
...
The processor is selected automatically:
...
|`true` (default)
|`processor-java-spring-hibernate` (full stack + Hibernate)

|`classic-*`, `performance-*`, `base`
|`false`
|`processor-java-spring-application` (full stack + plain identity)

|`reactive-*`
|n/a
|`processor-java-spring-application` (full stack; R2DBC persistence)

|`entity-only`
|`true`
|`processor-java-hibernate` (entity only + Hibernate)

|`entity-only`
|`false`
|`processor-java` (entity only + plain identity)
|===

==== Version Catalog Overrides

Override the auto-included dependencies via version catalog entries:
...
When present, catalog entries take precedence over the automatic selection.
...
=== Includes (Both Bundles)

* Everything in `spring-boot-native-complete-application` or `spring-boot-complete-library`:
...
* Application bundle also includes:
** GraalVM Native Image compilation (`nativeCompile`, `nativeTest`)
** Spring Boot AOT processing
** Ultima Nexus starter, annotations, and processor (auto-included)
...
Automatically aggregates reports and metrics from all subprojects into a single location.
Zero configuration needed — uses Gradle's project model to discover subprojects.
Works with any settings plugin or custom `settings.gradle.kts`.
...
** Code coverage (JaCoCo) — via `jacoco-report-aggregation`
** Test reports — via `test-report-aggregation`
** SBOM generation
** Plugin analysis
...
----
my-project/
├── build.gradle.kts           # Apply root bundle (aggregation is automatic)
├── settings.gradle.kts        # Any settings plugin or manual includes
├── my-modules/
│   ├── core/                  # Uses java-complete (has report plugin)
│   ├── api/                   # Uses java-complete (has report plugin)
│   └── web/                   # Uses spring-boot-complete (has report plugin)
└── example-project/
    └── demo/                  # Uses java-simple (no report plugin — skipped)
----
...
|`classic-backend`
|`ultima-nexus-jvm-application`
|Application shape: `classic-backend`, `classic-fullstack`, `performance-backend`, `performance-fullstack`, `reactive-backend`, `reactive-fullstack`, `base`, `entity-only`

|`ultimaNexusJvm.useHibernate`
|`true`
|`ultima-nexus-jvm-application`
|Use Hibernate generation
