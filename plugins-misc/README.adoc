...
* **Starter** (`implementation`) — predefined or plain, based on `ultimaNexusJvm.usePredefinedStarter`
* **Annotations** (`compileOnly`) — `@GenerateCrud`, `@GenerateApplication`, etc.
* **Processor** (`annotationProcessor`) — selected based on `ultimaNexusJvm.applicationType` and `ultimaNexusJvm.useHibernate`
...
|`true`
|Use the opinionated starter. Set `false` for the minimal starter. Ignored for `entity-only`.

|`ultimaNexusJvm.useHibernate`
|`true`
|Use Hibernate generation. Set `false` for plain JPA identity methods.
|===
